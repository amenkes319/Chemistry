...
public class BondTableController
{
	...
	{
		load(Compound.removeSubscript(combo.getSelectionModel().getSelectedItem().getText()));
	}
	
	private void load(String formula)
	{
	...
	public void displayCompounds(ArrayList<String> compounds)
	{
		...
		/*combo.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<Text>()
		{
            ...
            {
            	if(!(combo.getSelectionModel().getSelectedItem().getText().equals("No Results!") || combo.getSelectionModel().getSelectedItem().getText().equals("") || combo.getSelectionModel().isEmpty()))
            	{
	...
	private String[] IUPACName(String text)
	{
		return Compound.searchIUPACName(text);
	}
}
